...
// 豆瓣评论
function create_douban() {
    $labels = array(
        'name'               => _x( '豆瓣评论', 'Memory' ),
        'singular_name'      => _x( '豆瓣评论', 'Memory' ),
        'add_new'            => _x( '新建豆瓣评论', 'Memory' ),
        'add_new_item'       => __( '新建一个豆瓣评论', 'Memory' ),
        'edit_item'          => __( '编辑豆瓣评论', 'Memory' ),
        'new_item'           => __( '新豆瓣评论', 'Memory' ),
        'all_items'          => __( '所有豆瓣评论', 'Memory' ),
        'view_item'          => __( '查看豆瓣评论', 'Memory' ),
        'search_items'       => __( '搜索豆瓣评论', 'Memory' ),
        'not_found'          => __( '没有找到有关豆瓣评论', 'Memory' ),
        'not_found_in_trash' => __( '回收站里面没有相关豆瓣评论', 'Memory' ),
        'parent_item_colon'  => '',
        'menu_name'          => '豆瓣评论'
    );
    $args = array(
    ...
    register_post_type( 'douban', $args );
}
add_action( 'init', 'create_douban' );
add_action( 'add_meta_boxes', 'Memory_add_douban_box' );
function Memory_add_douban_box(){
    add_meta_box( 'Memory_douban_sticky', '置顶', 'Memory_douban_sticky', 'douban', 'side', 'high' );
}
function Memory_douban_sticky (){ ?>
    <input id="super-sticky" name="sticky" type="checkbox" value="sticky" <?php checked( is_sticky() ); ?> /><label for="super-sticky" class="selectit">置顶本条豆瓣评论</label>
<?php
}

function douban_info() {
    add_meta_box( 'douban_info', '豆瓣', 'douban_info_meta_box', 'douban', 'side', 'high' );
}
add_action( 'add_meta_boxes', 'douban_info' );
function douban_info_meta_box($post) {
    // 创建临时隐藏表单，为了安全
    wp_nonce_field( 'douban_info_meta_box', 'douban_info_meta_box_nonce' );
    // 获取之前存储的值
    $douban_id = get_post_meta( $post->ID, '_douban_id', true );
	$douban_score = get_post_meta( $post->ID, '_douban_score', true );
    ?>
    <input type="text" id="douban_id" name="douban_id" value="<?php echo esc_attr( $douban_id); ?>" placeholder="输入豆瓣id" >
    <input type="text" id="douban_score" name="douban_score" value="<?php echo esc_attr( $douban_score ); ?>" placeholder="输入你的评分" >
    <?php
}
add_action( 'save_post', 'douban_info_save_meta_box' );
function douban_info_save_meta_box($post_id){
    if ( ! isset( $_POST['douban_info_meta_box_nonce'] ) ) {
        return;
    }
    if ( ! wp_verify_nonce( $_POST['douban_info_meta_box_nonce'], 'douban_info_meta_box' ) ) {
        return;
    }
    if ( ! current_user_can( 'edit_post', $post_id ) ) {
        return;
    }
    if ( ! isset( $_POST['douban_id'] ) || ! isset( $_POST['douban_score'] ) ) {
        return;
    }
    $douban_id = sanitize_text_field( $_POST['douban_id'] );
    $douban_score = sanitize_text_field( $_POST['douban_score'] );
    update_post_meta( $post_id, '_douban_id', $douban_id );
    update_post_meta( $post_id, '_douban_score', $douban_score );
}
add_action("manage_posts_custom_column",  "douban_custom_columns");
add_filter("manage_edit-douban_columns", "douban_edit_columns");
function douban_custom_columns($column){
    global $post;
    switch ($column) {
        case "douban_id":
            echo get_post_meta( $post->ID, '_douban_id', true );
            break;
        case "douban_score":
            echo get_post_meta( $post->ID, '_douban_score', true );
            break;
    }
}
function douban_edit_columns($columns){
    $columns['douban_id'] = '豆瓣id';
	$columns['douban_score'] = '你的评分';
    return $columns;
}
